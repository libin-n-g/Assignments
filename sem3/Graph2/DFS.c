 ...
 * Program to perform Depth First Search in a graph.
 * Represent the graph using an adjacency list representation,
 * So the running time algorithm is linear in m+n, where m is the number of edges
 * and n is the number of vertices of the graph.
 * program also count the number of connected components in the given graph
 * and print the component number of each vertex */
...
int DepthFirstSearch(Nodeptr Nodes, int ComponentNo);
int main(void)
{
    int Num,NoConnetedNodes,i=0,j=1;
    Nodeptr Nodes=NULL;
    ...
    while(i < Num)
    {
        if((Nodes+i)->visit == 0)
        {
            NoConnetedNodes = DepthFirstSearch(Nodes+i,j);
            printf("\nConnected component %d contain %d Nodes\n\n",j,NoConnetedNodes);
            j++;
        }
        i++;
    }
...
int DepthFirstSearch(Nodeptr Nodes,int ComponentNo)
{
    int Num=0;
    listptr List=NULL;
    printf("Node %d is a member of Component %d\n",Nodes->nElement,ComponentNo);
    Nodes->visit=1;
    List=Nodes->Neigh;
    while(List!=NULL)
    {
        ...
        {
            Num=Num + DepthFirstSearch(List->Neighbours,ComponentNo);
        }
        List=List->Next;
    }
    return Num+1;
